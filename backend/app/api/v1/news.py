...
def list_news(region: str | None = None, limit: int = 50) -> list[NewsArticle]:
    ...
    return [NewsArticle.model_validate(i) for i in published[:limit]]


@router.get("/{article_id}", response_model=NewsArticle)
def get_news(article_id: str) -> NewsArticle:
    for item in news_store.read_all():
        if item["id"] == article_id:
            return NewsArticle.model_validate(item)
